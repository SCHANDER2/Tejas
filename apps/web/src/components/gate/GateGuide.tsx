          ...
          {subjects.map((subject) => (
            ...
            >
              ...
              {subject.youtubePlaylist && (
                <div className="bg-gradient-to-r from-red-950/40 to-orange-950/40 border border-red-800/40 rounded-xl p-3.5 space-y-2.5">
                  <div>
                    <span className="text-[10px] font-bold text-red-400 uppercase tracking-wider flex items-center gap-1">
                      <Play className="w-3 h-3 fill-current" /> Curated Lecture Playlist
                    </span>
                    <h5 className="font-bold text-xs text-white line-clamp-1 mt-0.5">{subject.youtubePlaylist.title}</h5>
                    <p className="text-[11px] text-slate-400">{subject.youtubePlaylist.channel} • {subject.youtubePlaylist.videoCount}</p>
                  </div>
                  <a
