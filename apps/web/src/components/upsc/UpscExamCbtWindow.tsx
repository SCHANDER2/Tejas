...
import { UpscQuestion, UpscPyqPaper, UpscModelPaper } from '../../data/upscData';
...
interface UpscExamCbtWindowProps {
  paperTitle: string;
  paperYearOrType: string;
  questions: UpscQuestion[];
  paperType?: 'GS1' | 'CSAT';
  onClose: () => void;
  rawPaperObj?: UpscPyqPaper | UpscModelPaper;
}

export default function UpscExamCbtWindow({
  paperTitle,
  paperYearOrType,
  questions,
  paperType = 'GS1',
  ...
  const calculateAnalysis = () => {
    ...
    questions.forEach((q, idx) => {
      ...
      if (userAns === undefined) unattemptedCount++;
        ...
        correctCount++;
        score += paperType === 'GS1' ? 2.0 : 2.5;
      } else {
        incorrectCount++;
        score -= paperType === 'GS1' ? 0.666 : 0.833;
      }
    });

    const maxMarks = 200;
    const targetCutoff = paperType === 'GS1' ? 90 : 66; // CSAT 33% qualifying
    ...
    const percentile = Math.min(99.9, Math.max(10, (score / maxMarks) * 100 * 1.8));
    const airRank = Math.max(10, Math.floor(650000 * (1 - percentile / 100)));
    ...
    <div className="fixed inset-0 z-50 bg-slate-900 text-slate-100 flex flex-col font-sans overflow-hidden">
      ...
      <div className="bg-[#003366] text-white px-6 py-3 border-b border-amber-500 flex items-center justify-between shadow">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-base leading-tight tracking-wide">UPSC CIVIL SERVICES PRELIMS ({paperType}) — CBT WINDOW</h1>
          ...
          <div className="flex-1 flex flex-col justify-between p-6 bg-white overflow-y-auto">
            <div>
              <div className="flex items-center justify-between border-b pb-3 mb-4">
                ...
                <span className="text-xs text-gray-600 font-bold">UPSC CSE ({paperType})</span>
              </div>
            ...
            <div className="pt-6 border-t border-gray-200 flex items-center justify-between mt-6 bg-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="ml-4 px-5 py-2 bg-[#003366] hover:bg-blue-900 text-white rounded text-xs font-bold shadow"
                >
                  Submit UPSC Paper
                </button>
              </div>
        ...
        <div className="flex-1 bg-slate-950 p-8 overflow-y-auto max-w-6xl mx-auto w-full">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl mb-8">
            <div className="flex items-center justify-between border-b border-slate-800 pb-6 mb-6">
              <div>
                <span className="text-xs font-bold text-amber-400 uppercase">UPSC CSE Prelims Calibration Report</span>
                <h2 className="text-2xl font-black text-white mt-1">{paperTitle} Result</h2>
              </div>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => exportUniversalExamPaperToPdf('upsc', paperTitle, paperYearOrType, questions)}
                ...
                <div className={`px-5 py-2 rounded-xl text-sm font-bold border ${
                  ...
                  {analysis.isPassed ? '🎉 QUALIFIED FOR MAINS EXAMINATION' : '⚠️ BELOW PRELIMS CUTOFF'}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <div className="bg-slate-800/80 p-5 rounded-xl border border-slate-700 text-center">
                <span className="text-xs text-slate-400 font-medium">UPSC Score</span>
                <p className="text-3xl font-black text-amber-400 mt-1">{analysis.score} <span className="text-xs text-slate-500">/ 200</span></p>
