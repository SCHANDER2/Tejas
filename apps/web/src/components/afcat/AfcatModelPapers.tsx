...
export default function AfcatModelPapers({ onStartQuiz }: { onStartQuiz?: () => void }) {
  ...
  const [activeCbtPaper, setActiveCbtPaper] = useState<AfcatModelPaper | null>(null);
    ...
    <div className="space-y-8 animate-fade-in">
            ...
            >
              ...
              <div className="space-y-2 pt-2 border-t border-[#e5e2d9]">
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-[#fcfcfb] hover:bg-[#e5e2d9]/50 text-[#262a2b] border border-[#e5e2d9] text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm"
                >
                  Launch Test Window <ExternalLink className="w-3.5 h-3.5 text-[#faa114]" />
                </button>
                <button
                ...
                {onStartQuiz && (
                  ...
                  >
                    Attempt as Micro Quiz <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                )}
              </div>
            </div>
      ...
      {activeCbtPaper && (
        <AfcatExamCbtWindow
          ...
          paperYearOrType={`Mock Paper ${activeCbtPaper.paperNumber}`}
